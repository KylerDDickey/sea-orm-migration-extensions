...
fn impl_migration_statement_tokens(ident: &Ident, up: &TokenStream, down: &TokenStream) -> TokenStream {
    quote! {
        ...
        impl MigrationStatementsTrait for #ident {
...
pub fn impl_create_table_migration_statements_tokens(ident: &Ident, options: &Vec<TokenStream>, col_stmts: &Vec<TokenStream>) -> TokenStream {
    let up = quote! {
